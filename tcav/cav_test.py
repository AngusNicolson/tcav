...
FLAGS = flags.FLAGS
flags.DEFINE_string(
    name="tcav_test_tmpdir", default="/tmp", help="Temporary directory for test files"
)
...
class CavTest(googletest.TestCase):
    def setUp(self):
        ...
        shape = (1, 3)
        self.acts = {
            concept: {self.bottleneck: np.tile(i * np.ones(shape), (4, 1))}
    ...
    def test_check_cav_exists(self):
        exists = self.cav.check_cav_exists(
            self.cav_dir, self.concepts, self.bottleneck, self.hparams
        )
    ...
    def test_perturb_act(self):
        perturbed = self.cav.perturb_act(
            np.array([1.0, 0, 1.0]), "concept1", operation=np.add, alpha=1.0
        )
        self.assertEqual(2.0, perturbed[0])
        self.assertEqual(2.0, perturbed[1])
        self.assertEqual(4.0, perturbed[2])
