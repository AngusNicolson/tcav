...
class TCAV(object):
    ...
    def compute_tcav_score(
        mymodel, target_class, concept, cav, examples, run_parallel=True, num_workers=20
    ):
        """Compute TCAV score.
        ...
        if run_parallel:
            ...
            directions = pool.map(
                lambda i: TCAV.get_direction_dir_sign(
                    mymodel, examples[i].to(device), cav, concept, class_id
                ),
                range(len(examples)),
            )
            pool.close()
            ...
            for i in range(len(examples)):
                ...
                if TCAV.get_direction_dir_sign(
                    mymodel, example, cav, concept, class_id
                ):
                    count += 1
    ...
    @staticmethod
    def get_directional_dir(mymodel, target_class, concept, cav, examples):
    ...
    def train_cavs(self, overwrite=False):
        ...
        for pair in self.pairs_to_test:
            ...
            for alpha in self.alphas:
                for bn in self.bottlenecks:
                    cav_path = Path(self.cav_dir) / (
                        CAV.cav_key(pair, bn, cav_hparams["model_type"], alpha).replace(
                            "/", "."
                        )
                        + ".pkl"
                    )
            ...
            if all([cav_path.exists() for cav_path in cav_paths]) and not overwrite:
                ...
                acts = self.activation_generator.process_and_load_activations(
                    self.bottlenecks,
                    pair,
                    overwrite=overwrite,
            ...
            for alpha in self.alphas:
                for bn in self.bottlenecks:
                    ...
                    print(
                        f"Finished training {cav_instance.get_key()} ({time.time() - t0:.1f} s)"
                    )
            # clean up
            del acts
    ...
    def _process_what_to_run_expand(
        self, num_random_exp=100, random_concepts=None, random_pairs=True
    ):
        ...
        self.pairs_to_test = (
            ...
            if self.relative_tcav or not random_pairs
