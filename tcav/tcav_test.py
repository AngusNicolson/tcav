...
class TcavTest(googletest.TestCase):
    def setUp(self):
        ...
        self.bottleneck = "bn"
        self.cav_dir = None
        self.hparams = {"model_type": "linear", "alpha": 0.01}
        ...
        self.mytcav_random_counterpart = TCAV(
            None,
            self.target,
            self.concepts,
            [self.bottleneck],
    ...
    def test_get_direction_dir_sign(self):
        self.assertFalse(
            TCAV.get_direction_dir_sign(
                self.mymodel, self.acts, self.cav, self.concepts[0], self.class_id, None
            )
        )
    ...
    def test_get_directional_dir(self):
        directional_dirs = TCAV.get_directional_dir(
            self.mymodel,
            self.target,
            self.concepts[0],
            self.cav,
        ...
        self.assertAlmostEqual(8.0, directional_dirs[0])
    ...
    def test__process_what_to_run_expand_random_counterpart(self):
        ...
        self.mytcav_random_counterpart._process_what_to_run_expand(num_random_exp=2)
        self.assertEqual(
            sorted(self.mytcav_random_counterpart.all_concepts),
            sorted(["t1", "c1", "c2", "random500_0", "random500_1", "random500_2"]),
        )
        self.assertEqual(
    ...
    def test__process_what_to_run_expand_specify_dirs_random_concepts(self):
        ...
        self.assertEqual(
            sorted(self.mytcav_random_counterpart.all_concepts),
            sorted(["t1", "c1", "c2", "random500_1", "random_dir1", "random_dir2"]),
        )
        self.assertEqual(
            sorted(self.mytcav_random_counterpart.pairs_to_test),
            sorted(
    ...
    def test__process_what_to_run_expand_relative_tcav(self):
        ...
        self.assertEqual(
            sorted(my_relative_tcav.all_concepts), sorted(["t1", "c1", "c2", "c3"])
        )
        self.assertEqual(
