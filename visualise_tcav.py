...
def main(args):
    ...
    act_generator = act_gen.ActivationGenerator(mymodel, source_json_path, activation_dir, JsonDataset,
                                                max_examples=max_examples, prefix=prefix, num_workers=num_workers,
                                                act_func=act_func)
    ...
    for i, example in enumerate(examples):
        ...
        for bn, bn_params in params.items():
            ...
            for concept in concepts:
                ...
                plt.savefig(img_out_dir / concept / f"{img_path.stem}_{bn}_directional_derivative.png")
                plt.close(fig)
    ...
    for values, name in zip([all_dot_acts, all_directional_derivatives], ["act_dot", "directional_derivative"]):
        for c, concept in enumerate(concepts):
            fig, axes = plt.subplots(len(bottlenecks), len(imgs), figsize=(2*len(imgs), 2*len(bottlenecks)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--working-dir", help="Where to save intermediate outputs", default="/tmp/tcav/example")
    parser.add_argument("--source-dir", help="The location of the images")
    parser.add_argument("--layers", help="A comma seperated list of the layers to run TCAV on",
                        default="layer4.0, layer4.1")
    parser.add_argument("--num-rand", help="The number of random experiments to run", type=int, default=30)
    parser.add_argument("--target", help="The target class name")
    parser.add_argument("--concepts", help="A comma seperated list of the concepts you wish to create CAVs for")
    parser.add_argument("--max-examples", help="The maximum number of images per concept", type=int, default=5)
    parser.add_argument("--num-workers", help="The number of cpu workers for dataloading", type=int, default=4)
    parser.add_argument("--exp-name", help="Experiment name (for saving results)", default="example")
    parser.add_argument("--act-func", help="Optional function to apply to the model activations", default=None)
